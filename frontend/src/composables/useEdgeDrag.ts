 ...
 * its viewport edge so it can be moved off the text/content it would otherwise cover.
 *
 * This is the single source of truth for vertical edge-handle dragging across the
 * whole stack. It lives in the `@ce` core so both the CE shell (e.g. the mobile
 * portal nav trigger in `MainLayout.vue`) and downstream apps (deepwork-terminal's
 * ResourceDrawer handle) import it from `@ce/composables/useEdgeDrag` — no copy-paste.
 *
 ...
 *    `shouldSuppressClick()` for an optional click guard). A movement over the
 *    threshold is a drag: we reposition, clamp to the viewport, persist, suppress click.
...
export function useEdgeDrag(options: EdgeDragOptions): EdgeDragBinding {
  ...
  // handle's @click (open drawer / toggle portal nav) doesn't trigger on reposition.
