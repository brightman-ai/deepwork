...
import { useWebSocketClient, type WebSocketClientOptions } from '@ce/composables/cli/useWebSocketClient'
import { useAgentIntel } from '@ce/composables/cli/useAgentIntel'
import type { AgentState, WSControlMessage } from '@ce/types/terminal'

export interface TerminalChannelOptions {
  /** Reactive session ID (null = no active session) */
  sessionId: Ref<string | null>
  /** Custom WS endpoint; defaults to /api/sessions/{id}/ws */
  endpoint?: string
...
export function useTerminalChannel(options: TerminalChannelOptions) {
  ...
  /** 原始 WSConnectionStatus (connecting/connected/disconnected/reconnecting/preempted) */
  const rawStatus = wsClient.status

  /** 网络统计 (RTT、带宽) */
  const netStats = wsClient.netStats
