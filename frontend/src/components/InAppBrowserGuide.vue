...
/**
 * InAppBrowserGuide — full-screen overlay shown when the app is opened inside an in-app webview
 * (WeChat / 企业微信 / QQ …). It points the user at the app's top-right "···" menu to reopen in the
 * system browser, and offers a copy-link fallback. WeChat blocks any programmatic jump to the system
 * browser, so guidance is the only reliable option; the overlay is dismissible (respect user agency)
 * and remembers the dismissal for the session so it does not nag on every navigation.
 *
 * SSOT: shared @ce component. Every host mounts it once (near the app root). Pure — reads only the
 * UA (via useInAppBrowser) + window.location — so it needs no host wiring.
 */
import { ref } from 'vue'
